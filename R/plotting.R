...
#' @param window.size.bp Window size in base-pairs that is used to smooth the curve.
#' @return A \code{\link[ggplot2:ggplot]{ggplot}} object.
#' @export
plotCombinatorialEntropy <- function(gr, genome, window.size.bp=NULL) {

	if (!is.null(window.size.bp)) {
		## Smoothing
		message("Calculating entropy in windows of ",window.size.bp,"bp ...", appendLF=F); ptm <- proc.time()
		...
		gr.extended$weighted.entropy <- width(gr.extended)*gr.extended$sum.entropy
		...
		gr <- GRanges(seqnames=df$seqnames, ranges=IRanges(start=df$start, end=df$end), sum.entropy=df$sum.weighted.entropy/df$sum.width)
	...
	# Delete entropy-matrix to avoid error in plotting function
	gr$entropy <- NULL
	...
	ggplt <- ggcytoband + ggbio::layout_karyogram(gr, aes_string(x='start', y='sum.entropy'), ylim=c(10,40), geom='line', color='red')
