...
heatmapCombinations <- function(hmm=NULL, marks=NULL, vertical=FALSE) {
  ...
  if (is.null(marks)) {
	...
	if (vertical) {
		ggplt <- ggplot(df) + geom_tile(aes_string(x='mark', y='combination', fill='emission')) + scale_fill_gradient(low='white', high='blue', guide=FALSE, limits=c(0,1)) + theme_bw() + theme(axis.text.x=element_text(angle=45,hjust=1))
	} else {
		ggplt <- ggplot(df) + geom_tile(aes_string(x='combination', y='mark', fill='emission')) + scale_fill_gradient(low='white', high='blue', guide=FALSE, limits=c(0,1)) + theme_bw() + theme(axis.text=element_text(angle=45,hjust=1))
	}
