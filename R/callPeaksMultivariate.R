...
#' @param use.states A data.frame with combinatorial states which are used in the multivariate HMM. This data.frame must be generated by the function \code{\link{stateBrewer}}. If both \code{use.states} and \code{num.states} are \code{NULL}, the maximum possible number of combinatorial states will be used.
...
callPeaksMultivariate <- function(modellist, use.states, num.states=NULL, chromosomes=NULL, eps=0.01, FDR=NULL, keep.posteriors=FALSE, num.threads=1, max.time=NULL, max.iter=NULL, checkpoint.after.iter=NULL, checkpoint.after.time=NULL, checkpoint.file=NULL, checkpoint.overwrite=TRUE, checkpoint.use.existing=FALSE, A.initial=NULL, keep.densities=FALSE, verbosity=1) {
	...
	if (!is.null(use.states)) {
		if (class(use.states)!='data.frame') stop("argument 'use.states' expects a data.frame generated by function 'stateBrewer'")
	}
