...
#' @param use.states A vector of combinatorial states which are used in the multivariate HMM. This vector can conveniently be generated by the function \code{\link{stateBrewer}}. If both \code{use.states} and \code{num.states} are \code{NULL}, the maximum possible number of combinatorial states will be used.
...
#' @param FDR False discovery rate. The default \code{NULL} means that the state with maximum posterior probability will be chosen, irrespective of its absolute probability.
...
#' @seealso \code{\link{chromstaR_multivariateHMM}}, \code{\link{callPeaksUnivariate}}, \code{\link{callPeaksReplicates}}
#' @examples
...
callPeaksMultivariate <- function(modellist, use.states, num.states=NULL, chromosomes=NULL, eps=0.01, FDR=NULL, keep.posteriors=FALSE, num.threads=1, max.time=NULL, max.iter=NULL, checkpoint.after.iter=NULL, checkpoint.after.time=NULL, checkpoint.file=NULL, checkpoint.overwrite=TRUE, checkpoint.use.existing=FALSE, A.initial=NULL, keep.densities=FALSE, verbosity=1) {
	...
	if (!is.null(use.states)) {
		if (check.nonnegative.integer.vector(use.states)!=0) stop("argument 'use.states' expects a vector of positive integers")
	}
	...
	if (!is.null(use.states) & !is.null(num.states)) {
		if (num.states > length(use.states)) {
			warning("The specified number of states (num.states) is larger than the number of specified states (use.states). Setting num.states = length(use.states).")
