...
#' @param modellist A list of \code{\link{uniHMM}}s generated by \code{\link{callPeaksUnivariate}}, e.g. \code{list(hmm1,hmm2,...)} or \code{c("file1","file2",...)}.
#' @param use.states A data.frame with combinatorial states which are used in the multivariate HMM. This data.frame must be generated by the function \code{\link{state.brewer}}. If both \code{use.states} and \code{num.states} are \code{NULL}, the maximum possible number of combinatorial states will be used.
...
#' @param chromosomes A vector specifying the chromosomes to use from the models in \code{modellist}. The default (\code{NULL}) uses all available chromosomes.
...
#'
callPeaksMultivariate <- function(modellist, use.states, num.states=NULL, chromosomes=NULL, eps=0.01, post.cutoff=NULL, keep.posteriors=FALSE, num.threads=1, max.time=NULL, max.iter=NULL, checkpoint.after.iter=NULL, checkpoint.after.time=NULL, checkpoint.file=NULL, checkpoint.overwrite=TRUE, checkpoint.use.existing=FALSE, A.initial=NULL, keep.densities=FALSE, verbosity=1) {
    ...
    repeat{
        ...
        ## Bin coordinates, posteriors and states
            ...
            if (!is.null(post.cutoff)) {
                result$bins$state <- factor(bin2dec(result$bins$posteriors >= 1-post.cutoff), levels=state.levels)
            } else {
        ...
        ## Add combinations
            if (!is.null(use.states)) {
                ...
                names(mapping) <- use.states$state
                result$bins$combination <- factor(mapping[as.character(result$bins$state)], levels=levels(use.states$combination))
                result$segments$combination <- factor(mapping[as.character(result$segments$state)], levels=levels(use.states$combination))
