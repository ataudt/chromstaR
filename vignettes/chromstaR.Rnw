...
# Let's be strict with the differences and get only clear ones
diff.segments <- diff.segments[diff.segments$differential.score > 0.999]
...
\begin{enumerate}
\item You can play with the ``differential.score'' (see section~\ref{sec:Task4}, step 3) and export only differences with a score close to 1.
