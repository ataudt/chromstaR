...
\Rfunction{Chromstar()} is the workhorse of the \Rpackage{chromstaR} package and produces all the files that are necessary for downstream analyses. Here is an explanation of the files and folders you will find in your ``outputfolder'':
\begin{enumerate}
\item 
\end{enumerate}
...
\subsection{The peak calls are too lenient. Can I adjust the strictness of the peak calling?}
