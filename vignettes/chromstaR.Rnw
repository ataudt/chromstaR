...
model <- adjustSensitivity(model, sensitivity=1e-4)
length(model$peaks); mean(width(model$peaks))
@
...
The strictness of the peak calling can be controlled with a sensitivity parameter. The Hidden Markov Model gives posterior probabilities for each peak, and based on these probabilites the model decides if a peak is present or not by picking the state with the highest probability. This way of peak calling leads to very lenient peak calls, and for some applications it may be desirable to obtain only very clear peaks. This can be achieved by setting the sensitivity parameter (which is a cutoff on the maximum posterior probability within each peak). To follow the below example, please first run step 1 and 2 from section~\ref{sec:Task4}.
...
model2 <- adjustSensitivity(model, sensitivity=1e-4)
...
cutoffs <- c(0.1, 0.1, 0.1, 0.1, 0.01, 0.01, 0.01, 0.01)
names(cutoffs) <- model$info$ID
print(cutoffs)
model2 <- adjustSensitivity(model, sensitivity=cutoffs)
## Compare the number and width of peaks before and after cutoff adjustment
length(model$segments); mean(width(model$segments))
length(model2$segments); mean(width(model2$segments))
@
...
\item Set a strict sensitivity (previous example) to obtain only high confidence peaks.
