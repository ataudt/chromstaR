...
plotTransitionProbs(model) + ggtitle('Transition probabilities')
...
\item \textbf{separate}: (This mode can't be selected and is automatically produced. It is a simple concatenation of the univariate peak calls.)
\end{itemize}
...
\begin{scriptsize}
<<faq_heatmapCombinations, results='markup', message=FALSE, eval=TRUE, fig.width=8, fig.height=4, fig.align='center', out.width='\\textwidth'>>=
