...
It is even possible to adjust the posterior cutoff for only one particular mark or condition. This might be useful in case the quality of a particular mark is not as expected and peak calls need to be tweaked.
