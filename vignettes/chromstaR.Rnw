...
<<multivariate_differential_preparation, results='show', message=FALSE, eval=TRUE>>=
#=== Step 1: Preparation ===
...
@
<<multivariate_differential_listfiles, results='show', message=FALSE, eval=TRUE>>=
...
<<multivariate_combinatorial_preparation, results='show', message=FALSE, eval=TRUE>>=
#=== Step 1: Preparation ===
## Prepare the file paths. Exchange this with your input and output directories.
inputfolder <- system.file("extdata","euratrans", package="chromstaRData")
...
<<multivariate_combinatorial_listfiles, results='show', message=FALSE, eval=TRUE, fig.width=4, fig.height=3>>=
## Results are stored in 'outputfolder' and can be loaded for further processing
list.files(outputfolder)
model <- get(load(file.path(outputfolder,'multivariate',
                            'multivariate_mode-mark_condition-liver.RData')))
# Plot transition probabilities and read count correlation
plot(model, type='transitionMatrix')
plot(model, type='correlation')
@
...
<<multivariate_combinatorial_analysis_plot1, results='show', message=FALSE, eval=TRUE, fig.width=6, fig.height=4>>=
...
@
<<multivariate_combinatorial_analysis_plot2, results='show', message=FALSE, eval=TRUE, fig.width=8, fig.height=4>>=
...
<<combined_preparation, results='show', message=FALSE, eval=TRUE>>=
#=== Step 1: Preparation ===
## Prepare the file paths. Exchange this with your input and output directories.
inputfolder <- system.file("extdata","euratrans", package="chromstaRData")
...
Chromstar(inputfolder, experiment.table=experiment_table,
          ...
          prefit.on.chr='chr12', mode='mark')
@
<<combined_listfiles, results='show', message=FALSE, eval=TRUE>>=
## Results are stored in 'outputfolder' and can be loaded for further processing
list.files(outputfolder)
model <- get(load(file.path(outputfolder,'combined', 'combined_mode-mark.RData')))
@

<<combined_analysis, results='show', message=FALSE, eval=TRUE>>=
## Obtain all genomic regions where the two tissues have different states
segments <- model$combined.segments
diff.segments <- segments[segments$liver != segments$lv]
exportGRanges(diff.segments, trackname='differential chromatin states',
              filename=tempfile())
@
\end{scriptsize}
